          ...
          return (
            <div key={task.id} className="py-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div><p className={`font-semibold text-sm ${isLocked ? 'text-slate-400' : 'text-slate-900 dark:text-white'}`}>{task.title}</p><p className="text-[11px] text-slate-400">{task.project}</p></div>
              </div>
              <div className="flex items-center gap-2">
                ...
                {isLocked ? (
                  <span className="p-1.5 rounded-lg text-slate-300 cursor-not-allowed"><Lock className="w-3.5 h-3.5" /></span>
                ) : (
