...
import { ListTodo, Clock, Wallet, ShieldCheck } from 'lucide-react';
import StatCard from '../components/shared/StatCard';
import SearchBar from '../components/shared/SearchBar';
import TaskCard from '../components/tasks/TaskCard';
import AntiIdleGuard from '../components/tracker/AntiIdleGuard';
  ...
  return (
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Anti-Idle Guard" value="8s CUTOFF" icon={ShieldCheck} iconBg="bg-amber-50" iconColor="text-emerald-600" darkMode={darkMode} />
      </div>
        ...
        <div className="lg:col-span-2">
          <div className={`${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} rounded-2xl border p-6 shadow-sm`}>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-700 gap-3">
              <div><h2 className="text-lg font-bold text-slate-900 dark:text-white">Task Center</h2><p className="text-xs text-slate-500">Start a task to begin tracking</p></div>
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-700 mt-2">
              {filteredTasks.length === 0 ? <div className="p-8 text-center text-slate-400 text-sm">No tasks found.</div> : filteredTasks.map((task) => (
                <TaskCard key={task.id} task={task} activeTaskId={activeTaskId} isTracking={isTracking} onStart={onStart} onSubmit={onSubmit} onView={onView} darkMode={darkMode} activeSeconds={taskActiveSeconds[task.id]} />
              ))}
            </div>
          </div>
        </div>
        <div className="space-y-6">
          <AntiIdleGuard isKeyboardActive={isKeyboardActive} isMouseActive={isMouseActive} isDualInputActive={isDualInputActive} darkMode={darkMode} />
        </div>
      </div>
    </>
  );
